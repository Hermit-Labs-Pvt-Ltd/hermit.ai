...
demo = {
    ...
    showNotification: function(from, align) {
        ...
        $.notify({
            icon: "notifications",
            message: "Welcome to <b>Tega Dashboard."
...
function panel(panelinfo)
{
    ...
    if(panelinfo.status == 0)
    {
        if(laststate[panelinfo.device_id-1]==1)
        {
            $( panelslno ).data( "background-color", "green");
            $(panelslno).css( "background", "linear-gradient(60deg, #66bb6a, #43a047)");
            $(panelslno).find('i').text( "check_circle_outline");
         ...
         laststate[panelinfo.device_id-1]=0;
    }

    else
    {
        if(laststate[panelinfo.device_id-1]==0)
        {
        ...
        laststate[panelinfo.device_id-1]=1;
    }
}
